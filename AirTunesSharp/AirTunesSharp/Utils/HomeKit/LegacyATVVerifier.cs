...
namespace AirTunesSharp.Utils.HomeKit
{
    internal class LegacyATVVerifier
    {
        ...
        public static string pair_setup_aes_key(string K)
        {
            ...
            {
                byte[] hash = shaM.ComputeHash(Encoding.ASCII.GetBytes("Pair-Setup-AES-Key").Concat(NumUtil.HexStringToByteArray(K)).ToArray());
        ...
        public static string pair_setup_aes_iv(string K)
        {
            using (SHA512 shaM = new SHA512Managed())
            {
                byte[] hash = shaM.ComputeHash(Encoding.ASCII.GetBytes("Pair-Setup-AES-IV").Concat(NumUtil.HexStringToByteArray(K)).ToArray());
        ...
        public static string pair_verify_aes_key(string shared)
        {
            using (SHA512 shaM = new SHA512Managed())
            {
                byte[] hash = shaM.ComputeHash(Encoding.ASCII.GetBytes("Pair-Verify-AES-Key").Concat(NumUtil.HexStringToByteArray(shared)).ToArray());
                hash = hash.Skip(0).Take(16).ToArray();
                return Convert.ToHexString(hash);
            }
        }
        ...
        public static string pair_verify_aes_iv(string shared)
        {
            using (SHA512 shaM = new SHA512Managed())
            {
                byte[] hash = shaM.ComputeHash(Encoding.ASCII.GetBytes("Pair-Verify-AES-IV").Concat(NumUtil.HexStringToByteArray(shared)).ToArray());
                hash = hash.Skip(0).Take(16).ToArray();
                return Convert.ToHexString(hash);
            }
        ...
        public static Dictionary<string,string> confirm(string a, string K)
        {
            ...
            var plaintextBytes = NumUtil.HexStringToByteArray(a_pub(a));
            var ciphertext = new byte[NumUtil.HexStringToByteArray(a_pub(a)).Length];
            var tag = new byte[System.Security.Cryptography.AesGcm.TagByteSizes.MaxSize];
            using (var aes = new System.Security.Cryptography.AesGcm(NumUtil.HexStringToByteArray(key)))
            {
                aes.Encrypt(NumUtil.HexStringToByteArray(iv), plaintextBytes, ciphertext, tag);
            } ;
        ...
        public static string signature(string shared, string atv_data, string signed)
        {
                ...
                aes.Key = NumUtil.HexStringToByteArray(pair_verify_aes_key(shared));
                aes.IV = NumUtil.HexStringToByteArray(pair_verify_aes_iv(shared));
                aes.BlockSize = 128;
