    ...
    public class Credentials
    {
        ...
        public byte[] encrypt(byte[] message)
        {
            ...
            while (offset < total)
            {
                ...
                byte[] s1length_bytes = BitConverter.GetBytes(Convert.ToUInt16(length));
                (byte[] s1ct, byte[] s1tag) = Encryption.EncryptAndSeal(message.Skip(offset).Take(length).ToArray(), s1length_bytes, (new byte[] { 0x00, 0x00, 0x00, 0x00 }).Concat(BitConverter.GetBytes(Convert.ToUInt64(this.encryptCount))).ToArray(), this.writeKey);
        ...
        public byte[] decrypt(byte[] message)
        {
            ...
            {   
                ...
                byte[] decrypted = Encryption.VerifyAndDecrypt(cipherText, hmac, lengthbytes, (new byte[] { 0x00, 0x00, 0x00, 0x00 }).Concat(BitConverter.GetBytes(Convert.ToUInt64(this.decryptCount))).ToArray(), this.readKey);
                this.decryptCount += 1;
